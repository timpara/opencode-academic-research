 ...
 *
 * Logs a short notice to the OpenCode app log on session start so a user
 * can confirm the Academic Research Skills suite is active.
 *
 * Skill auto-discovery in OpenCode is metadata-driven (skills/<name>/SKILL.md
 * frontmatter), so this plugin does NOT need to inject a system message to
 * make the skills visible to the model. It exists for observability and
 * parity with the upstream announce script.
 */
